...
# Description: function implementing a random (exhaustive) search for unpartitioned geographical regions
#
...
#         R - number of candidate solutions that should be explored
#
# Outputs: x_incumbent - vector with entries in {0, 1} and cardinality n representing the incumbent solution at the last iteration (solution with highest objective encountered)
...
function random_search(D::Array{Float64, 2}, c::Float64, n::Float64, R::Int64)
  ...
  Dx_candidate = Array{Float64}(undef, W, 1)
  y_candidate = Array{Float64}(undef, W, 1)
...
#################### Top-to-Bottom Greedy Heuristic #######################

# Description: function implementing a greedy heuristic (Berger et al, 2020) with random selection of locations leading to same objective reduction for unpartitioned geographical regions (single cardinality constraint)
#
# Comments: 1) types of inputs should match those declared in argument list
...
function ttb_greedy_heuristic(D::Array{Float64,2}, c::Float64, n::Float64)
...
function time_ttb_greedy_heuristic(D::Array{Float64,2}, c::Float64, n::Float64)
  @time ttb_greedy_heuristic(D, c, n)
end

#################### Classic Greedy Heuristic #######################

# Description: function implementing a classic greedy algorithm (Nemhauser et al, 1978) with random selection of locations leading to same objective gain for unpartitioned geographical regions (single cardinality constraint)
#
# Comments: 1) types of inputs should match those declared in argument list
#           2) at every iteration, randomisation is used to select the location that should be removed from the locations set when several locations are "tied", i.e., removing each of them leads to the same decrease in objective value
#
...
# Description: function implementing the stochastic greedy algorithm for submodular maximization (Mirzasoleiman et al, 2015) for unpartitioned geographical regions (single cardinality constraint)
#
# Comments: 1) types of inputs should match those declared in argument list
#           2) at every iteration, randomisation is used to select the location that should be removed from the locations set when several locations are "tied", i.e., removing each of them leads to the same decrease in objective value
#
...
#         c - global criticality threshold
#         n - number of sites to deploy
#         epsilon - factor (<<1) defining the quality of approximation ratio 1 - 1/e - epsilon and the size of the random subset s = (L/n) x log 1/epsilon
#
# Outputs: x_incumbent - vector with entries in {0, 1} and cardinality n representing the incumbent solution at the last iteration
#          LB_incumbent - objective value of incumbent solution, provides a lower bound on optimal objective
#
